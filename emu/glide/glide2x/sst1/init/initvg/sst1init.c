...
#define DE10_STAGE_PRINTF(...) do { fprintf(stderr, __VA_ARGS__); fflush(stderr); } while(0)
#else
#define DE10_STAGE_PRINTF(...) do { } while(0)
#endif

#ifdef DE10_BACKEND
static void de10StageFile(const char *stage) {
...
#else
static void de10StageFile(const char *stage) { (void)stage; }
#endif
#include "rcver.h"
...
FX_EXPORT FxU32 * FX_CSTYLE sst1InitMapBoard(FxU32 BoardNumber)
{
    ...
    de10StageFile("sst1InitMapBoard: enter");
    if(firstTime) {
        /* Find "voodoo.ini" file if it exists... */
        sst1InitUseVoodooFile = sst1InitVoodooFile();

        de10StageFile("sst1InitMapBoard: before numBoardsInSystem");

        if(!(boardsInSystem = sst1InitNumBoardsInSystem()))
            return(NULL);
        de10StageFile("sst1InitMapBoard: after numBoardsInSystem");
    }
        ...
        for(j=0; j<SST1INIT_MAX_BOARDS; j++) {
          ...
          FxU32 deviceID = 1;                /* Find only sst1 boards */
...
#ifndef DIRECTX
            de10StageFile("sst1InitMapBoard: before pciMapCardMulti");
            ...
            sst = pciMapCardMulti(vendorID, deviceID, sizeOfCard, 
                &sst1InitDeviceNumber, j, 0);
#endif /* DIRECTX */
            de10StageFile("sst1InitMapBoard: after pciMapCardMulti");
            if(sst) {
                de10StageFile("sst1InitMapBoard: before PCI cfg reads");
    ...
    de10StageFile("sst1InitMapBoard: before fbiInit3 read");
    ...
    de10StageFile("sst1InitMapBoard: exit");
    return(sst);
}
...
FX_EXPORT FxBool FX_CSTYLE sst1InitRegisters(FxU32 *sstbase)
{
    ...
    DE10_STAGE_PRINTF("[de10] stage: after assert grx+pci reset\n");
    de10StageFile("sst1InitRegisters: after assert grx+pci reset");
    sst1InitIdleFBINoNOP(sstbase);

    /* Unreset PCI FIFO... */
    ISET(sst->fbiInit0, IGET(sst->fbiInit0) & ~SST_PCI_FIFO_RESET);
    DE10_STAGE_PRINTF("[de10] stage: after release pci fifo reset\n");
    de10StageFile("sst1InitRegisters: after release pci fifo reset");
    sst1InitIdleFBINoNOP(sstbase);
    ...
    DE10_STAGE_PRINTF("[de10] stage: after release graphics reset\n");
    de10StageFile("sst1InitRegisters: after release graphics reset");
    sst1InitIdleFBINoNOP(sstbase);
    ...
    DE10_STAGE_PRINTF("[de10] stage: after reset init regs to defaults\n");
    de10StageFile("sst1InitRegisters: after reset init regs");
    ...
    DE10_STAGE_PRINTF("[de10] stage: after pci init enable\n");
    de10StageFile("sst1InitRegisters: after pci init enable");

    /* Determine DAC Type */
    DE10_STAGE_PRINTF("[de10] stage: before dac detect\n");
    de10StageFile("sst1InitRegisters: before dac detect");
    if(sst1InitDacDetect(sstbase) == FXFALSE) {
        INIT_PRINTF(("sst1InitRegisters(): Could not detect DAC...\n"));
        return(FXFALSE);
    }
    DE10_STAGE_PRINTF("[de10] stage: after dac detect\n");
    de10StageFile("sst1InitRegisters: after dac detect");
    ...
    DE10_STAGE_PRINTF("[de10] stage: after trex0 init0 write\n");
    de10StageFile("sst1InitRegisters: after trex0 init0");
    sst1InitIdleFBINoNOP(sstbase);
    ISET(SST_TREX(sst,0)->trexInit1, sst1CurrentBoard->tmuInit1[0]);
    DE10_STAGE_PRINTF("[de10] stage: after trex0 init1 write\n");
    de10StageFile("sst1InitRegisters: after trex0 init1");
    sst1InitIdleFBINoNOP(sstbase);
...
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitRegisters: after setResolution, before idle\n");
    fflush(stderr);
#endif
    sst1InitIdleFBINoNOP(sstbase);
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitRegisters: after post-setResolution idle\n");
    fflush(stderr);
#endif
    ...
    if(!GETENV(("SST_SKIP_INIT_NOP")))
        ISET(sst->nopCMD, 0x1); /* Clear fbistat registers after clearing screen */
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitRegisters: after fastfillCMD, before idle\n");
    fflush(stderr);
#endif
    sst1InitIdleFBINoNOP(sstbase);
#if defined(DE10_BACKEND) || defined(SIM_BACKEND)
    fprintf(stderr, "[de10] sst1InitRegisters: after fastfill idle\n");
    fflush(stderr);
#endif
    de10StageFile("sst1InitRegisters: after fastfill idle");

    de10StageFile("sst1InitRegisters: before FillDeviceInfo");
    ...
    de10StageFile("sst1InitRegisters: after FillDeviceInfo");
    ...
    de10StageFile("sst1InitRegisters: exit");
    return(FXTRUE);
}
...
#ifdef DE10_BACKEND
    INIT_PRINTF(("sst1InitShutdown(): DE10 backend using minimal headless shutdown\n"));
    return(FXTRUE);
#endif
