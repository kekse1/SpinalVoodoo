...
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grBufferClear: start color=0x%x alpha=0x%x depth=0x%x\n",
          color, alpha, depth);
  fflush(stderr);
#endif
...
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grBufferClear: end\n");
  fflush(stderr);
#endif
  GR_END_SLOPPY();
} /* grBufferClear */
...
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grBufferSwap: command issued effective=%d vsync=%d\n",
          swapInterval, vSync);
  fflush(stderr);
#endif
...
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grBufferSwap: end total=%u\n",
          _GlideRoot.stats.bufferSwaps);
  fflush(stderr);
#endif
...
GR_ENTRY(grColorCombine, void, ( GrCombineFunction_t function, GrCombineFactor_t factor, GrCombineLocal_t local, GrCombineOther_t other, FxBool invert ))
{
...
#ifdef DE10_BACKEND
  fprintf(stderr,
          "[de10] grColorCombine: enter fn=%d factor=%d local=%d other=%d invert=%d\n",
          function,
          factor,
          local,
          other,
...
#ifdef DE10_BACKEND
  fprintf(stderr,
          "[de10] grColorCombine: oldTex=0x%x newPath=0x%08x ccTex=%u acTex=%u\n",
          oldTextureEnabled ? 1U : 0U,
          fbzColorPath,
  ...
  if(oldTextureEnabled != (fbzColorPath & SST_ENTEXTUREMAP)) {
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grColorCombine: before texture transition nop\n");
#endif
    P6FENCE_CMD( GR_SET(hw->nopCMD,0) );
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grColorCombine: after texture transition nop\n");
#endif
  }

  /* update register */
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grColorCombine: before fbzColorPath write\n");
#endif
  GR_SET( hw->fbzColorPath, fbzColorPath );
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grColorCombine: after fbzColorPath write\n");
#endif
  gc->state.fbi_config.fbzColorPath = fbzColorPath;

  /* setup paramIndex bits */
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grColorCombine: before _grUpdateParamIndex\n");
#endif
  _grUpdateParamIndex();
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grColorCombine: after _grUpdateParamIndex\n");
#endif
...
#ifdef DE10_BACKEND
  fprintf(stderr, "[de10] grGlideShutdown: start initialized=%d\n",
          _GlideRoot.initialized);
  fflush(stderr);
#endif
  ...
  if ( _GlideRoot.initialized ) {
    ...
    }
    {
      ...
      for( i = 0; i < _GlideRoot.hwConfig.num_sst; i++ ) {
        grSstSelect( i );
#ifdef DE10_BACKEND
        fprintf(stderr, "[de10] grGlideShutdown: closing sst=%d\n", i);
        fflush(stderr);
#endif
        grSstWinClose();
      }
...
#ifdef DE10_BACKEND
      fprintf(stderr, "[de10] grGlideShutdown: before initClose\n");
      fflush(stderr);
#endif
      initClose();
#ifdef DE10_BACKEND
      fprintf(stderr, "[de10] grGlideShutdown: after initClose\n");
      fflush(stderr);
#endif
...
#ifdef DE10_BACKEND
      fprintf(stderr, "[de10] grGlideShutdown: done\n");
      fflush(stderr);
#endif
      GR_END();
    }
