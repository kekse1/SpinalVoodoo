...
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grSstWinClose: before initIdle\n");
    fflush(stderr);
    initIdle();
    fprintf(stderr, "[de10] grSstWinClose: after initIdle\n");
    fflush(stderr);
#else
    initIdle();
#endif
...
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grSstWinClose: before initDisableTransport\n");
    fflush(stderr);
#endif
    initDisableTransport();
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grSstWinClose: after initDisableTransport\n");
    fflush(stderr);
#endif
...
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grSstWinClose: before initRestoreVideo\n");
    fflush(stderr);
#endif
    initRestoreVideo();
#ifdef DE10_BACKEND
    fprintf(stderr, "[de10] grSstWinClose: after initRestoreVideo\n");
    fflush(stderr);
#endif
#if (GLIDE_PLATFORM & GLIDE_OS_WIN32)
    if (gc->oemInit) {
